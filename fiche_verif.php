...
include($root.'includes/init_donnees.php');
...
for ($i = 1; $i < $nFacture+1; $i++)
  ...
  if ($action == 'maj') {
    ...
    $liste = [
      ...
      'fabricant_id'=>"<label for='fabricant_id'>Fabricant :</label><select name='fabricant_id'>".$listeFabricants."</select>",
      'facture_id'=>"<label for='facture_id'>Facture :</label><select name='facture_id'>".$listeFactures."</select>",
      "milieutexte"=>"' ",
      'fintexte'=>"'>",
      'finnum'=>">",
      'required'=>' required ',
      'datemax'=>$donnees['date_max'],
      'libelle'=>"<label for='libelle'>Libellé :</label><input name='libelle' type='text' required values=".$donnees['libelle'].">",
      'date_debut'=>"<label for='date_debut'>Mise en service :</label><input name='date_debut' type='date' values=".$donnees['date_debut'].">",
      'date_debut'=>"<label for='date_debut'>Mise en service :</label><input name='date_debut' type='date' values=".$donnees['date_debut'].">"
    ];
  }
...
<hr>
  ...
  <form  enctype="multipart/form-data" method='post' action="fiche_verif.php">
    <table>
      <tbody>
        <tr>
          <td><?php echo "Référence :<br>".$donnees['reference']?></td>
          <td colspan=1><label for='photo'>Adresse photo :</label><input name="photo" value=<?php echo $donnees['photo']?>></td>
        </tr>
        <tr>
          <td><?php echo "<label for='libelle :'>Libellé :</label><input name='libelle' size=60 type='text' required value="."'".$donnees['libelle']."'>"?></td>
        ...
        <tr>
          <td><label for='nb_elements'>Nombre d'éléments :</label><input type="number" name="nb_elements" value=<?php echo $donnees['nb_elements']?> min=0 max=<?php echo $donnees['nb_elements_initial']?>></td>
        </tr>
        <tr>
        ...
        <tr></tr>
          <td colspan = 2>
            ...
            <textarea name="remarques" cols = 100 rows=5 ><?php echo $donnees['remarques'] ?></textarea> </td>
        </tr>
      </tbody>
      <tfoot>
        <tr>
        ...
        <td align='right'>
          <input type="submit" name="envoyer" value="Mise à jour de la fiche"></td>
      </tfoot>
    </table>
  </form>
